...
let index = 0;
const mappedAnswer = answers.reduce((acc, row) => {
  ...
  if(row === "") {
    ...
    acc[index].personCount = acc[index].personCount + 1;
    row
      .split("")
      .map(char => {
        acc[index] = {
          ...acc[index],
          [char]: (acc[index][char] ? acc[index][char] : 0) + 1
        }
      })
...
const count = Object.values(mappedAnswer).reduce((acc, answer) => {
  const numberOfAnswers = Object.keys(answer).length - 1;
  acc = acc + numberOfAnswers;
  return acc;
}, 0)

const count2 = Object.values(mappedAnswer).reduce((acc, answer) => {
  const personCount = answer.personCount;

  const numberOfAnswers = Object.keys(answer).reduce((acc, key) => {
    if(key !== 'personCount') {
      if(answer[key] === personCount) {
        acc = acc + 1;
      }
    }
    return acc;
