...
const first = passwords.reduce((acc, passwordObj) => {
  const arr = passwordObj.split(" ");
  const [minimum, maximum] = arr[0].split("-")
  const [character] = arr[1].split(":")
...
const second = passwords.reduce((acc, passwordObj) => {
  const arr = passwordObj.split(" ");
  const [firstPos, secondPos] = arr[0].split("-")
  const [character] = arr[1].split(":")
  const password = arr[2];
  ...
  if(
    characters[firstPos-1] === character && characters[secondPos-1] !== character ||
    characters[firstPos-1] !== character && characters[secondPos-1] === character) {
    acc = acc + 1;
  }
