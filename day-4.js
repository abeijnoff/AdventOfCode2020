...
const passports = [];
passportsBrutto.reduce((acc, row) => {
  if(row === "") {
    passports.push(acc);
    return {};
  } else {
...
const validationRules = {
  ...
  hcl: /^#[0-9|a-f][0-9|a-f][0-9|a-f][0-9|a-f][0-9|a-f][0-9|a-f]$/,
...
passports.map(passport => {
  const validatedPassport = Object.keys(passport).reduce((acc, key) => {
    acc[key] = true;
    return acc;
  }, {
    ...validationObj
  ...
  const valid = Object.values(validatedPassport).reduce((acc, value) => {
    if (!value) {
      return value;
    }
    return acc;
...
passports.map(passport => {
  const validatedPassport = Object.entries(passport).reduce((acc, [key, value]) => {

    if(key !== "cid") {
      const regex = validationRules[key];
      if(regex.test(value)) {
        acc[key] = true; 
      }
    }
