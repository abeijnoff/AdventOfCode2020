...
const baseBagObject = bagRules.reduce((acc, bag) => {
...
const splitBagRules = rule => {
  const bagObject = {
    containBags: {}
  }
  const parts = rule.split(",");

  const type = parts[0].trim().split("contain")[0].trim().split("bags")[0].trim()
  const contain1 = parts[0].trim().split("contain")[1].trim()

  if(contain1.split(" ").length > 3) {
    const containBags = contain1.split("bag")[0].trim();
    const containBagsSplit = containBags.split(" ");
    bagObject.containBags[`${containBagsSplit[1]} ${containBagsSplit[2]}`] = containBagsSplit[0];
  }

  if(parts.length > 1) {
    parts.map((part, index) => {
      if(index > 0) {
        const containBags = part.split("bag")[0].trim();
        const containBagsSplit = containBags.split(" ");
        bagObject.containBags[`${containBagsSplit[1]} ${containBagsSplit[2]}`] = containBagsSplit[0];
      }
    })
...
const bagObject = bagRules.reduce((acc, rule) => {
  const bag = splitBagRules(rule);

  acc[bag.type].containBags = {...bag.containBags}
  Object.keys(bag.containBags).map(key => {
    acc[key].containedBy[bag.type] = true;
  })
  return acc;
}, baseBagObject)
...
const findContainers = (bag, type) => {
  const current = bag[type];
  const containedBy = Object.keys(current.containedBy);

  if(containedBy.length > 0) {
    const recursiveContainers = Object.keys(current.containedBy).reduce((acc, key) => {
      acc = acc.concat(findContainers(bag, key));
      return acc;
    }, []);
    return containedBy.concat(recursiveContainers)
  }
  else {
    return containedBy;
  }
...
const containers = findContainers(bagObject, "shiny gold");

function onlyUnique(value, index, self) {
  return self.indexOf(value) === index;
}

const bagCount = containers.filter(onlyUnique).length

const countBags = bags => {
  return Object.entries(bags).reduce((acc, [key, value]) => {
    acc = acc + parseInt(value) + (parseInt(value) * countBags(bagObject[key].containBags))
    return acc;
  }, 0)
}

const totalCount = countBags(bagObject["shiny gold"].containBags);


console.log(bagCount, totalCount)